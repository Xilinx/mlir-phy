...
module @MM_2x2 {
  ...
  %stream60_mm2s0     = physical.stream<[0, 1]>(){ aie.tile = [6, 0], aie.port = "O.DMA.0" }: !physical.stream<i32>
  %stream60_mm2s1     = physical.stream<[2, 3]>(){ aie.tile = [6, 0], aie.port = "O.DMA.1" }: !physical.stream<i32>
  %stream70_mm2s0     = physical.stream<[4, 5]>(){ aie.tile = [7, 0], aie.port = "O.DMA.0" }: !physical.stream<i32>
  %stream70_s2mm0     = physical.stream<[6, 7]>(){ aie.tile = [7, 0], aie.port = "I.DMA.0" }: !physical.stream<i32>
  %stream63_s2mm0     = physical.stream<[0]>(){ aie.tile = [6, 3], aie.port = "I.DMA.0" }: !physical.stream<i32>
  %stream63_s2mm1     = physical.stream<[2]>(){ aie.tile = [6, 3], aie.port = "I.DMA.1" }: !physical.stream<i32>
  %stream64_s2mm0     = physical.stream<[1]>(){ aie.tile = [6, 4], aie.port = "I.DMA.0" }: !physical.stream<i32>
  %stream64_s2mm1     = physical.stream<[3]>(){ aie.tile = [6, 4], aie.port = "I.DMA.1" }: !physical.stream<i32>
  %stream64_mm2s0     = physical.stream<[6]>(){ aie.tile = [6, 4], aie.port = "O.DMA.0" }: !physical.stream<i32>
  %stream73_s2mm0     = physical.stream<[0]>(){ aie.tile = [7, 3], aie.port = "I.DMA.0" }: !physical.stream<i32>
  %stream73_s2mm1     = physical.stream<[4]>(){ aie.tile = [7, 3], aie.port = "I.DMA.1" }: !physical.stream<i32>
  %stream74_s2mm0     = physical.stream<[1]>(){ aie.tile = [7, 4], aie.port = "I.DMA.0" }: !physical.stream<i32>
  %stream74_s2mm1     = physical.stream<[5]>(){ aie.tile = [7, 4], aie.port = "I.DMA.1" }: !physical.stream<i32>
  %stream74_mm2s0     = physical.stream<[7]>(){ aie.tile = [7, 4], aie.port = "O.DMA.0" }: !physical.stream<i32>

  %stream60_mm2s0_out = physical.ostream(%stream60_mm2s0: !physical.stream<i32>)
  physical.stream_dma(%stream60_mm2s0_out: !physical.ostream<i32>) {
  ...
  %stream60_mm2s1_out = physical.ostream(%stream60_mm2s1: !physical.stream<i32>)
  physical.stream_dma(%stream60_mm2s1_out: !physical.ostream<i32>) {
  ...
  %stream70_mm2s0_out = physical.ostream(%stream70_mm2s0: !physical.stream<i32>)
  physical.stream_dma(%stream70_mm2s0_out: !physical.ostream<i32>) {
  ...
  %stream70_s2mm0_in  = physical.istream(%stream70_s2mm0: !physical.stream<i32>)
  physical.stream_dma(%stream70_s2mm0_in: !physical.istream<i32>) {
  ...
  %stream63_s2mm0_in  = physical.istream(%stream63_s2mm0: !physical.stream<i32>)
  physical.stream_dma(%stream63_s2mm0_in: !physical.istream<i32>) {
    %0 = physical.stream_dma_connect(%lock63_0[0->1], %buf63_0[0:1024]: memref<1024xi32>, %0)
  } { aie.tile = [6, 3], aie.engine = "S2MM0" }

  %stream63_s2mm1_in  = physical.istream(%stream63_s2mm1: !physical.stream<i32>)
  physical.stream_dma(%stream63_s2mm1_in: !physical.istream<i32>) {
    %0 = physical.stream_dma_connect(%lock63_1[0->1], %buf63_1[0:1024]: memref<1024xi32>, %0)
  } { aie.tile = [6, 3], aie.engine = "S2MM1" }

  %stream64_s2mm0_in  = physical.istream(%stream64_s2mm0: !physical.stream<i32>)
  physical.stream_dma(%stream64_s2mm0_in: !physical.istream<i32>) {
    %0 = physical.stream_dma_connect(%lock64_0[0->1], %buf64_0[0:1024]: memref<1024xi32>, %0)
  } { aie.tile = [6, 4], aie.engine = "S2MM0" }

  %stream64_s2mm1_in  = physical.istream(%stream64_s2mm1: !physical.stream<i32>)
  physical.stream_dma(%stream64_s2mm1_in: !physical.istream<i32>) {
    %0 = physical.stream_dma_connect(%lock64_1[0->1], %buf64_1[0:1024]: memref<1024xi32>, %0)
  } { aie.tile = [6, 4], aie.engine = "S2MM1" }

  %stream64_mm2s0_out = physical.ostream(%stream64_mm2s0: !physical.stream<i32>)
  physical.stream_dma(%stream64_mm2s0_out: !physical.ostream<i32>) {
    %0 = physical.stream_dma_connect<6>(%lock64_2[1->0], %buf64_2[0:1024]: memref<1024xi32>, %0)
  } { aie.tile = [6, 4], aie.engine = "MM2S0" }

  %stream73_s2mm0_in  = physical.istream(%stream73_s2mm0: !physical.stream<i32>)
  physical.stream_dma(%stream73_s2mm0_in: !physical.istream<i32>) {
    %0 = physical.stream_dma_connect(%lock73_0[0->1], %buf73_0[0:1024]: memref<1024xi32>, %0)
  } { aie.tile = [7, 3], aie.engine = "S2MM0" }

  %stream73_s2mm1_in  = physical.istream(%stream73_s2mm1: !physical.stream<i32>)
  physical.stream_dma(%stream73_s2mm1_in: !physical.istream<i32>) {
    %0 = physical.stream_dma_connect(%lock73_1[0->1], %buf73_1[0:1024]: memref<1024xi32>, %0)
  } { aie.tile = [7, 3], aie.engine = "S2MM1" }

  %stream74_s2mm0_in  = physical.istream(%stream74_s2mm0: !physical.stream<i32>)
  physical.stream_dma(%stream74_s2mm0_in: !physical.istream<i32>) {
    %0 = physical.stream_dma_connect(%lock74_0[0->1], %buf74_0[0:1024]: memref<1024xi32>, %0)
  } { aie.tile = [7, 4], aie.engine = "S2MM0" }

  %stream74_s2mm1_in  = physical.istream(%stream74_s2mm1: !physical.stream<i32>)
  physical.stream_dma(%stream74_s2mm1_in: !physical.istream<i32>) {
    %0 = physical.stream_dma_connect(%lock74_1[0->1], %buf74_1[0:1024]: memref<1024xi32>, %0)
  } { aie.tile = [7, 4], aie.engine = "S2MM1" }

  %stream74_mm2s0_out = physical.ostream(%stream74_mm2s0: !physical.stream<i32>)
  physical.stream_dma(%stream74_mm2s0_out: !physical.ostream<i32>) {
    %0 = physical.stream_dma_connect<7>(%lock74_2[1->0], %buf74_2[0:1024]: memref<1024xi32>, %0)
  } { aie.tile = [7, 4], aie.engine = "MM2S0" }

  %stream60_mm2s0_in  = physical.istream(%stream60_mm2s0: !physical.stream<i32>)
  %stream60_mm2s1_in  = physical.istream(%stream60_mm2s1: !physical.stream<i32>)
  %stream70_mm2s0_in  = physical.istream(%stream70_mm2s0: !physical.stream<i32>)
  %stream64_mm2s0_in  = physical.istream(%stream64_mm2s0: !physical.stream<i32>)
  %stream74_mm2s0_in  = physical.istream(%stream74_mm2s0: !physical.stream<i32>)
  %stream70_s2mm0_out = physical.ostream(%stream70_s2mm0: !physical.stream<i32>)
  %stream63_s2mm0_out = physical.ostream(%stream63_s2mm0: !physical.stream<i32>)
  %stream63_s2mm1_out = physical.ostream(%stream63_s2mm1: !physical.stream<i32>)
  %stream64_s2mm0_out = physical.ostream(%stream64_s2mm0: !physical.stream<i32>)
  %stream64_s2mm1_out = physical.ostream(%stream64_s2mm1: !physical.stream<i32>)
  %stream73_s2mm0_out = physical.ostream(%stream73_s2mm0: !physical.stream<i32>)
  %stream73_s2mm1_out = physical.ostream(%stream73_s2mm1: !physical.stream<i32>)
  %stream74_s2mm0_out = physical.ostream(%stream74_s2mm0: !physical.stream<i32>)
  %stream74_s2mm1_out = physical.ostream(%stream74_s2mm1: !physical.stream<i32>)
  physical.stream_hub(
      %stream60_mm2s0_in, %stream60_mm2s0_in, %stream70_mm2s0_in, %stream64_mm2s0_in, %stream74_mm2s0_in,
      %stream70_s2mm0_out, %stream63_s2mm0_out, %stream63_s2mm1_out, %stream64_s2mm0_out, %stream64_s2mm1_out,
      %stream73_s2mm0_out, %stream73_s2mm1_out, %stream74_s2mm0_out, %stream74_s2mm1_out)
...
^bb:
  ...
  physical.core @kernel(%buf63_0, %lock63_0, %buf63_1, %lock63_1, %buf63_2, %lock63_2, %buf63_3, %lock63_3)
    { aie.tile = [6, 3] }
    : (memref<1024xi32>, !physical.lock, memref<1024xi32>, !physical.lock,
       memref<1024xi32>, !physical.lock, memref<1024xi32>, !physical.lock) -> !physical.core
